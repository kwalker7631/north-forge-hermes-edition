...
Trigger: /audit or /chk, or a request to review, check, test, compare, debug, clean up, find drift in, or verify consistency of an existing prompt, KB, article, response, or draft. Use this skill unless a code-maintenance file such as CLAUDE.md is specifically requested (that's a different kind of review, outside this skill's scope).
