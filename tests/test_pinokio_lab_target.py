...
Covers the reconciled rule from EXCALIBUR.md / PINOKIO.md / PINOKIO-on-drive.md:
a North Forge marker hard-blocks only on an Excalibur-class (small) volume;
...
def test_blocks_north_forge_volume_excalibur_class(tmp_path):
    """A small (Excalibur-class) stick with a marker is blocked outright,
...
def test_blocks_north_forge_volume_marker_in_parent_excalibur_class(tmp_path):
    root = tmp_path
...
def test_excalibur_ceiling_boundary_is_inclusive(tmp_path):
    """Exactly at --excalibur-max-gb (default 100) still counts as Excalibur-class."""
    (tmp_path / "north-forge.cmd").write_text("echo hi")
    candidate = tmp_path / "PinokioHome"
...
def test_excalibur_max_gb_is_configurable(tmp_path):
    (tmp_path / "north-forge.cmd").write_text("echo hi")
    candidate = tmp_path / "PinokioHome"
    ...
    # ...but explicitly raising the ceiling can still classify it Excalibur-side.
    raised_result = target.validate_target(
        candidate, excalibur_max_gb=2000, disk_usage=_fake_disk_usage(free_gb=900, total_gb=1000)
    )
...
def test_cli_exit_code_blocked(tmp_path):
    ...
    candidate = tmp_path / "PinokioHome"
    # --excalibur-max-gb set absurdly high so this is deterministic regardless
...
def test_cli_does_not_block_marker_above_default_excalibur_ceiling(tmp_path):
