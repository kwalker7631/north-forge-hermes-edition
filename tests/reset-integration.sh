...
make_fixture() {
    ...
    mkdir -p "$fixture"
    cp "$REPO_ROOT/toggle-mode.sh" "$fixture/"
...
printf 'RESET\nRumpleStiltskin\nYES\n' | bash "$SUCCESS/toggle-mode.sh" > "$SUCCESS/output.txt"
for target in "${TARGETS[@]}"; do
...
if printf 'RESET\nRumpleStiltskin\nNO\n' | bash "$REFUSAL/toggle-mode.sh" > "$REFUSAL/output.txt"; then
...
if printf 'RESET\nRumpleStiltskin\nYES\n' | bash "$INCOMPLETE/toggle-mode.sh" > "$INCOMPLETE/output.txt" 2>&1; then
