...
Write-Host "  3. Once it's running, check that project skills were picked up: run '/skills list --source local' and"
Write-Host "     look for kb-builder. It's discovered from .hermes/skills/ (confirmed by reading the actual Hermes"
Write-Host "     source) and auto-trusted by the launcher - if it's still not showing, say so exactly rather than"
Write-Host "     assuming it's a repeat of the same issue."
